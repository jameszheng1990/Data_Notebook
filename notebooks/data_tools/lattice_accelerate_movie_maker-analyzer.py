...
exp_name = 'lattice_accelerate_movie_maker'
    
def fit_Gaussian(x, x0, w, amp, const):
    return amp * np.exp(-pow(x - x0, 2)/(2*pow(w, 2))) + const

def process_lattice_accelerate_movie_maker_ccd(settings):
    shot = settings['shot']
...
def plot_lattice_accelerate_movie_maker_ccd(data, settings):
